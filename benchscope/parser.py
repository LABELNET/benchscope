...
_add("concurrency", "concurrency", r"Maximum request concurrency:\s+(\d+)")
...
_add("mean", "req_per_s", r"Request throughput \(req/s\):\s+([\d.]+)")
...
def parse_metrics(output: str) -> dict:
    ...
    for kind, metric, pattern in _PATTERNS:
        ...
        if m:
            key = metric if kind == "concurrency" else f"{metric}_{kind}"
            try:
    ...
    # 兼容只有 mean 没有 p99 的情况：p99 缺省回退 mean
    for metric in ("ttft", "tpot", "itl"):
        mean_key = f"{metric}_mean"
        p99_key = f"{metric}_p99"
        if mean_key in metrics and p99_key not in metrics:
