...
description: >-
  ...
  using benchscope. Covers engine selection (built-in benchscope /
  sglang-0.5.10 / custom versions) with environment validation,
  OpenAI-compatible service configuration, sglang.bench_serving parameters with
  descriptions, random/sharegpt/custom datasets, concurrency & request rate,
...
- You need to **add a new SGLang bench version** → use the
...
- **Either** a local SGLang install for native engines (`python -m sglang.bench_serving --help`),
...
| `sglang-0.5.10` | sglang (subprocess CLI) | `python -m sglang.bench_serving` | `torch>=2.0` + `sglang==0.5.10` |
| custom `sglang-<ver>` | sglang | `python -m sglang.bench_serving` | `torch` + `sglang` at that version |

**Rules (enforced):**
- Native engines (`kind=sglang`) **must** pass environment validation
  (`torch` + `sglang` version spec + module availability) — otherwise **Next is blocked** and
