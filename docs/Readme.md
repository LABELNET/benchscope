...
>
> **git 提交规范（强制）**：提交描述一律使用**英文**、**简短总结**（Conventional Commits，如 `feat:` / `fix:` / `docs:` / `refactor:` / `test:` 前缀），禁止中文或冗长描述；发布后同步 docs 状态也按此规范提交（如 `docs: mark vX.Y.Z as released`）。详见 `rules/Development.md` §4。
