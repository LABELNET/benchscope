...
> **Release 功能清单规则（强制）**：GitHub Release 说明为**按功能总结的中英功能清单**，**不能直接搬迭代记录**（不带迭代标题、不带时间、非逐迭代记录）；发布前须在 `VERSION_x_y_z.md` 维护「版本功能清单（Release Notes）」双语区块（每条 = `- **中文** / **English**（细节）`），详见 `rules/Development.md` §5。
>
