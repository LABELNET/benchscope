...
| 文档 | 内容 |
| --- | --- |
| [versions/VERSION_1_1_1.md](versions/VERSION_1_1_1.md) | **1.1.1（开发中）**：当前迭代版本，后续开发内容按时间顺序追加 |
| [versions/VERSION_1_1_0.md](versions/VERSION_1_1_0.md) | **1.1.0（已发布 2026-09-05）**：性能实时面板 / 单请求快照、Sessions 体验、Dashboard Overview 重构 + Envs 网络补全、Settings/Datas 收敛（主/次版本，推 PyPI） |
| [versions/VERSION_1_0_9.md](versions/VERSION_1_0_9.md) | **1.0.9（并入 1.1.0）**：1.0.8 之后的开发周期版本，未独立发布，迭代 1–25 明细随 1.1.0 发布（归档） |
| [versions/VERSION_1_0_8.md](versions/VERSION_1_0_8.md) | **1.0.8（已发布 2026-09-01）**：独立精度测试模块（Accuracy）+ 性能页增强（补丁发布，未推 PyPI） |
...
| [rules/Design.md](rules/Design.md) | 设计规范：UI / 字体 / 颜色，**对齐 Ant Design 设计语言**（价值观 / 设计令牌 / 8px 网格 / 24 栅格） |
...
Skills（给 AI 消费的技能包）的**说明文档归口**；技能可分发产物在包内 `benchscope/skills/`。
...
| [options/](options/) | **CLI 选项手册**：`benchscope perf` / `eval` 等子命令的参数与说明 |
