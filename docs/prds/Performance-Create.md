...
- **阈值模式**：每组显示阈值条件 `TPOT mean ≤ X ms`、`Output throughput ≤ Y tok/s`（可编辑）
...
dataset: { type: random, length_pairs: [[inputLen, outputLen, "IxO", case_id], ...] }  # 第 4 元素为唯一组 id
concurrency_list: 并发模式 = 首组请求数；阈值模式 = [1]
request_rate: inf | follow
tpot_threshold_ms / output_throughput_threshold（阈值模式）
