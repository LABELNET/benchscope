...
| 模块 | 路由 | 说明 |
| --- | --- | --- |
| Dashboard（仪表盘） | `/dashboard` | 统计卡片（总测试次数 / 进行中任务 / 平均 TPOT / 最佳模型）+ 历史运行记录列表，详情面板复用均值/P99 分析 |
| Performance（任务化性能测试） | `/performance` `/performance/create` `/performance/:taskId` | 任务列表（卡片视图）+ 新建任务表单 + 任务详情页（左侧实时进度 + bench 终端，右侧实时双语表格 + 六条曲线，顶部状态栏含开始/停止/重试） |
| Accuracy（精度测试） | `/accuracy` | 占位页，为 v5.0 精度测试版本预留，1.0.5 不实现功能逻辑 |
| Sessions（会话） | `/sessions` | SSE 流式对话，会话列表 + 模型选择 + 系统提示词 + Markdown 渲染；会话持久化到 `~/.benchscope/sessions/` |
| Settings（设置） | `/settings` | 分区设置：通用（主题/语言/默认框架/阈值/目录/bench 命令）、推理服务 API、GPU；Plugins 区为后续版本占位 |
...
### 4.4 任务 API（`/api/tasks`，1.0.5 新增）

任务化性能测试的核心接口，定义于 `benchscope/server/api_tasks.py`。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/tasks` | 获取所有任务列表（`{ tasks: [...] }`） |
| GET | `/api/tasks/{task_id}` | 获取单个任务详情快照（不存在返回 404） |
| POST | `/api/tasks` | 创建任务，body 为 `CreateTaskRequest`（framework / model / tokenizer / dataset / concurrency_list / gpu / request_rate / tpot_threshold_ms / precision / curated / extra_args），返回 `{ ok, task_id, task }` |
| POST | `/api/tasks/{task_id}/start` | 启动任务（不存在 404；已在运行 409） |
| POST | `/api/tasks/{task_id}/stop` | 停止任务 |
| DELETE | `/api/tasks/{task_id}` | 删除任务 |
| POST | `/api/tasks/{task_id}/preview` | 预览将要执行的 bench 命令，返回 `{ ok, commands, count }` |

任务持久化到 `~/.benchscope/tasks/<task_id>.json`，服务启动时扫描恢复 `running` 状态任务。所有 WebSocket 消息扩展 `task_id` 字段以便前端按任务路由。

### 4.5 会话 API（`/api/sessions`，1.0.5 新增）

会话与 SSE 流式对话接口，定义于 `benchscope/server/api_sessions.py`。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/sessions` | 获取会话列表（`{ sessions: [...] }`） |
| POST | `/api/sessions` | 新建会话，body 为 `CreateSessionRequest`（title / model / system_prompt） |
| GET | `/api/sessions/{session_id}` | 获取会话详情（含消息历史，不存在 404） |
| DELETE | `/api/sessions/{session_id}` | 删除单个会话 |
| DELETE | `/api/sessions` | 清空全部会话 |
| POST | `/api/sessions/{session_id}/chat` | 发送消息并获取流式回复，body 为 `ChatRequest`（message / model），返回 `text/event-stream`（SSE，`data: <chunk>\n\n`） |

会话持久化到 `~/.benchscope/sessions/`，每个会话一个 JSON 文件。

### 4.6 Dashboard 统计 API（`/api/dashboard`，1.0.5 新增）

Dashboard 仪表盘的统计接口，定义于 `benchscope/server/api_dashboard.py`。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/dashboard/stats` | 返回 Dashboard 统计：`total_runs`（总测试次数）、`running_tasks`（进行中任务数，来自 TaskManager）、`avg_tpot`（最近一次完成测试的平均 TPOT）、`best_model`（TPOT 最低的模型名）。数据源为 `logs/` 下各运行目录的 `run.json` |
...
```
┌──────────────────────────────────────────────────────────┐
│  TopBar：品牌 + 5 栏导航（Dashboard / Performance /         │
│         Accuracy / Sessions / Settings） + 状态指示灯       │
├──────────┬───────────────────────────────────────────────┤
│ 左侧导航  │               内容区                          │
│（固定，   │  ┌─────────────────────────────────────────┐ │
│ 仅详情页  │  │  各页面内容（内部滚动）                    │ │
│  显示）   │  │                                         │ │
│           │  │  DashboardView / PerformanceView /        │ │
│           │  │  TaskDetailView / CreateTaskView /       │ │
│           │  │  AccuracyView / SessionsView /           │ │
│           │  │  SettingsView                            │ │
│           │  │                                         │ │
└──────────┴──┴─────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────┘
...
{ path: '/performance/create',   component: CreateTaskView },
{ path: '/performance/:taskId',  component: TaskDetailView, props: true },
{ path: '/accuracy',             component: AccuracyView },
...
| 页面 | 核心组件 | 说明 |
| --- | --- | --- |
| **DashboardView** | StatsCards | 统计卡片：总测试次数 / 进行中任务 / 平均 TPOT / 最佳模型（数据源 `/api/dashboard/stats`） |
| | RunTable | 历史运行记录列表（复用 `/api/logs/runs`），含筛选/搜索/排序 |
| | RunDetailPanel | 详情面板（复用：指标汇总 / 均值 / P99 / 日志文件四 Tab） |
| **PerformanceView** | TaskCard | 任务卡片列表（网格视图），显示任务 ID / 模型 / 框架 / 数据集 / 状态 / 进度 + 进入/开始/停止/删除/重试 |
| **CreateTaskView** | TaskCreateForm | 新建任务分步表单：选择模型与框架 → 配置测试参数 → 确认与命令预览 |
| **TaskDetailView** | TaskStatusBar | 顶部状态栏：任务信息 + 服务状态 + 控制按钮（开始/停止/重试/命令预览/编辑配置） |
| | TaskProgress | 左侧上半：圆形进度条 + 当前用例/并发/状态 |
| | TaskTerminal | 左侧下半：深色终端，实时流式显示 bench 输出 |
| | MetricsTable | 右侧上半：复用，双语表头实时更新 |
| | MetricsCharts | 右侧下半：复用，6 条曲线 |
| | AnalysisBlock | 任务完成后切换：均值/P99 分析块 |
| **AccuracyView** | 占位卡片 | `a-result` 风格「功能规划中」，预计版本 v5.0 |
| **SessionsView** | SessionList | 左侧会话列表（新建/清空/切换/高亮） |
| | ChatPanel | 右侧对话区：模型选择 + 系统提示词 + 消息列表（Markdown）+ 输入框（Enter 发送 / Shift+Enter 换行） |
| | MessageBubble | 消息气泡（用户/助手交替） |
| **SettingsView** | GeneralSettings | 通用：主题（亮/暗/跟随系统）、语言（中/英）、默认框架、TPOT 阈值、目录、bench 命令 |
| | ApiSettings | 推理服务 API：Base URL / Endpoint / API Key / 额外请求头 / 测试连接 |
| | GpuSettings | GPU：自动检测 / 手动回退 / 刷新 |
| **通用** | TopBar | 顶部导航栏（5 项 + 状态指示灯） |
| | StatusBadge | 状态指示灯组件 |
...
| **v1.0.1 ~ v1.0.3** | ✅ 已发布 | 补丁版本：README 双语化、打包元数据、源码链接更新 |
| **v1.0.4** | ✅ 已发布 | 补丁版本：README 调整、打包元数据更新 |
| **v1.0.5** | 🚀 已发布 | v2.0 UI 大改：5 栏导航（Dashboard / Performance / Accuracy / Sessions / Settings）、任务化 Performance（多任务持久化）、Sessions SSE 流式对话、Accuracy 占位页、Settings 分区（通用 / 推理服务 API / GPU）、i18n（中/英）、亮/暗/跟随系统主题 |
| **v2.0** | 🔜 规划中 | 多模态模型性能测试（图像/视频输入） |
| **v3.0** | 规划中 | 全模态模型性能测试（音频/视频等） |
| **v4.0** | 规划中 | 世界模型性能测试 |
| **v5.0** | 规划中 | 常见数据集精度测试（1.0.5 的 Accuracy 页为该版本占位） |
...
│   │   ├── state.py              # 全局状态单例（含 TaskManager / SessionManager）
│   │   ├── status.py             # 推理服务状态监控
│   │   ├── ws.py                 # WebSocket 广播 Hub（扩展 task_id 路由）
│   │   ├── api_config.py         # 配置/模型/GPU/状态 API
│   │   ├── api_test.py           # 旧版单测试启停与进度 API（兼容）
│   │   ├── api_tasks.py          # 任务化测试 API（CRUD + 启停 + 预览）— 1.0.5 新增
│   │   ├── api_sessions.py       # 会话 API（CRUD + SSE chat）— 1.0.5 新增
│   │   ├── api_dashboard.py      # Dashboard 统计 API — 1.0.5 新增
│   │   ├── api_logs.py           # 日志/数据集管理 API
│   │   └── test_manager.py       # 旧版单测试执行管理器（api_test 使用）
│   ├── task_manager.py            # 任务管理器（多任务 + 持久化）— 1.0.5 新增
│   ├── session_manager.py        # 会话管理器（存储 + SSE 流式对话）— 1.0.5 新增
│   └── webui/                    # 前端构建产物（打包分发）
├── web/                          # 前端源码
│   ├── src/
