...
- **沙箱环境启动「沙箱开发模式」，主机环境启动「主机开发模式」**（两套独立：启动方式 / 数据目录 / 端口占位，互不干扰）。
- **每个涉及代码更新的任务执行完毕，须重启对应环境的开发模式**以加载最新代码。
- 主机环境用 `./scripts/dev.sh start|stop|status`（mock :8001 + 前端构建 + 后端 :8080）。
- 沙箱环境因 bwrap `--unshare-pid` 无法管理主进程，需自起 mock + 后端（后台 job + `mktemp` 独立数据目录），详见 `agents/Memory.md` §10。
- 明确写入项目记忆：`agents/Memory.md`（强制约定 #11 + §10 开发模式）。
